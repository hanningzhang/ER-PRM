...
def select_sample(args,sample,model,tokenizer,candidate_tokens,step_tag_id):
    ...
    with torch.no_grad():
        ...
        for i in range(scores.shape[0]):
            ...
            if len(step_scores):
                scores_list.append(min(step_scores))  ## process reward
                #scores_list.append(step_scores[-1]) ## outcome reward
                scores_save.append(step_scores)
            else:
                scores_list.append(0)
        chosen_idx = scores_list.index(max(scores_list))
        reject_idx = scores_list.index(min(scores_list))
        chosen = sample['answers'][chosen_idx]
        rejected = sample['answers'][reject_idx]
        return_data = {"prompt":prompt,"chosen":chosen,"rejected":rejected}
        return return_data, scores_save
