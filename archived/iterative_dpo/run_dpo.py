...
class ScriptArguments:
    ...
    num_train_epochs: Optional[float] = field(default=2, metadata={"help": "max number of training epochs"})
    ...
    save_steps: Optional[float] = field(default=100, metadata={"help": "the saving frequency"})
    ...
    mask_prompt: Optional[bool] = field(default=True, metadata={"help": "mask prompt"})
...
def prepare_hallucination_dataset(
    data_dir: str = "/home/xiongwei/data/helpful/rm/rm1003.json",
    eot_token="",
    length_penalty=0,
) -> Dataset:
...
if __name__ == "__main__":
    ...
    script_args.learning_rate *= (script_args.total_iteration-script_args.current_iteration+1)/script_args.total_iteration
    ...
    train_dataset = prepare_hallucination_dataset(
        data_dir=f"{script_args.train_dir}/result.json",
    ...
    # eval_dataset = prepare_hallucination_dataset(
    ...
    #eval_dataset = Dataset.from_dict(train_dataset[int(0.9*length):])
    #train_dataset = Dataset.from_dict(train_dataset[:int(0.9*length)])
