...
if __name__ == "__main__":
    ...
    #step_tag = 'ки' 
    processed_dataset = []
    #dataset = dataset[int((args.split/2)*len(dataset)):int(((args.split+1)/2)*len(dataset))]
    # if args.split == 0:
    ...
    llm = LLM(model=args.completion_model_name_or_path,tensor_parallel_size=args.tensor_parallel_size, enforce_eager=True, dtype = "float16",gpu_memory_utilization=0.9,swap_space=128)
    print("------------")
    ...
    completions_list = []
    for batch_num in range(length,len(format_prompt),3000):
